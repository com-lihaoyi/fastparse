...
//        assert(parser == !(" "))
    ...
    'cuts{
      ...
      'nestedcut{
        ...
        // Failing after the nested either with the cut inside still prevents
        // backtracking, because cuts are permanent.
        val Result.Failure(11, _, _) = Parse("worldxamaba").read(parse2(_))
      }
      'repnocut{
        def alpha[_: P] = P( CharIn("a-z") )
