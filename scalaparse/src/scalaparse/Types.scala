...
trait Types extends Core{
  ...
  def CompoundType[$: P]  = {
    def Refinement = P( OneNLMax ~ `{` ~/ Dcl.repX(0, Semis, Int.MaxValue, -1) ~ `}` )
  ...
  def SimpleType[$: P]: P[Unit] = {
    ...
    def BasicType = P( TupleType | Literals.NoInterp.Literal | TypeId ~ ("." ~ `type`).?  | _wildcard )
  ...
  def TypeArg[$: P]: P[Unit] = {
    ...
    P((Id | _wildcard) ~ TypeArgList.? ~ TypeBounds ~ CtxBounds)
  }
