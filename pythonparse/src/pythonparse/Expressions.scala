...
object Expressions {
  ...
  def NUMBER[_p: P]: P[Ast.expr.Num] = P( Lexical.floatnumber | Lexical.longinteger | Lexical.integer | Lexical.imagnumber ).map(Ast.expr.Num)
  ...
  def list_comp[_p: P] = P( list_comp_contents ).map(Ast.expr.ListComp.tupled)
  def generator[_p: P] = P( list_comp_contents ).map(Ast.expr.GeneratorExp.tupled)

  def lambdef[_p: P]: P[Ast.expr.Lambda] = P( kw("lambda") ~ varargslist ~ ":" ~ test ).map(Ast.expr.Lambda.tupled)
  ...
  def named_argument[_p: P] = P( NAME ~ "=" ~ test  ).map(Ast.keyword.tupled)
  ...
  def yield_expr[_p: P]: P[Ast.expr.Yield] = P( kw("yield") ~ testlist.map(tuplize).? ).map(Ast.expr.Yield)
