...
object Expressions {
  ...
  def list_comp[_p: P] = P( list_comp_contents ).map { case (a, b) => Ast.expr.ListComp(a, b) }
  def generator[_p: P] = P( list_comp_contents ).map { case (a, b) => Ast.expr.GeneratorExp(a, b) }

  def lambdef[_p: P]: P[Ast.expr.Lambda] = P( kw("lambda") ~ varargslist ~ ":" ~ test ).map { case (a, b) => Ast.expr.Lambda(a, b) }
  ...
  def dictorsetmaker[_p: P]: P[Ast.expr] = {
    ...
    def dict_comp = P(
      (dict_item ~ comp_for.rep(1)).map { case (a, b, c) => Ast.expr.DictComp(a, b, c) }
    )
    def set: P[Ast.expr.Set] = P( test.rep(1, ",") ~ ",".? ).map(Ast.expr.Set.apply)
    def set_comp = P( test ~ comp_for.rep(1) ).map { case (a, b) => Ast.expr.SetComp(a, b) }
  ...
  def named_argument[_p: P] = P( NAME ~ "=" ~ test  ).map { case (a, b) => Ast.keyword(a, b) }
