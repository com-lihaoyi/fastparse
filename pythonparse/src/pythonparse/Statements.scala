...
class Statements(indent: Int){
  ...
  def space[_p: P] = P( CharIn(" \n") )
  def NEWLINE[_p: P]: P0 = P( "\n" | End )
  def ENDMARKER[_p: P]: P0 = P( End )

  def single_input[_p: P]: P[Seq[Ast.stmt]] = P(
    NEWLINE.map(_ => Nil) |
    simple_stmt |
  ...
  def spaces[_p: P] = P( (Lexical.nonewlinewscomment.? ~~ "\n").repX(1) )
  def file_input[_p: P]: P[Seq[Ast.stmt]] =
    P( spaces.? ~ stmt.repX(0, spaces) ~ spaces.? ).map(_.flatten)
  def eval_input[_p: P]: P[Ast.expr] = P( testlist ~ NEWLINE.rep ~ ENDMARKER ).map(tuplize)
  ...
  def decorator[_p: P]: P[Ast.expr] = P( "@" ~/ dotted_name ~ ("(" ~ arglist ~ ")" ).?  ~~ Lexical.nonewlinewscomment.? ~~ NEWLINE).map{
  ...
  def decorators[_p: P] = P( decorator.rep )
  def decorated[_p: P]: P[Ast.stmt] = P( decorators ~ (classdef | funcdef) ).map{case (a, b) => b(a)}
  def classdef[_p: P]: P[Seq[Ast.expr] => Ast.stmt.ClassDef] =
    P( kw("class") ~/ NAME ~ ("(" ~ testlist.? ~ ")").?.map(_.toSeq.flatten.flatten) ~ ":" ~~ suite ).map{
      case (a, b, c) => Ast.stmt.ClassDef(a, b, c, _)
    }


  def funcdef[_p: P]: P[Seq[Ast.expr] => Ast.stmt.FunctionDef] = P( kw("def") ~/ NAME ~ parameters ~ ":" ~~ suite ).map{
    case (name, args, suite) => Ast.stmt.FunctionDef(name, args, suite, _)
  }
  def parameters[_p: P]: P[Ast.arguments] = P( "(" ~ varargslist ~ ")" )

  def stmt[_p: P]: P[Seq[Ast.stmt]] = P( compound_stmt.map(Seq(_)) | simple_stmt )

  def simple_stmt[_p: P]: P[Seq[Ast.stmt]] = P( small_stmt.rep(1, sep = ";") ~ ";".? )
  def small_stmt[_p: P]: P[Ast.stmt] = P(
    print_stmt  | del_stmt | pass_stmt | flow_stmt |
    import_stmt | global_stmt | exec_stmt | assert_stmt | expr_stmt
  )
  def expr_stmt[_p: P]: P[Ast.stmt] = {
  ...
  def augassign[_p: P]: P[Ast.operator] = P(
  ...
  def print_stmt[_p: P]: P[Ast.stmt.Print] = {
  ...
  def del_stmt[_p: P] = P( kw("del") ~~ " ".rep ~~ exprlist ).map(Ast.stmt.Delete.apply)
  def pass_stmt[_p: P] = P( kw("pass") ).map(_ => Ast.stmt.Pass)
  def flow_stmt[_p: P]: P[Ast.stmt] = P( break_stmt | continue_stmt | return_stmt | raise_stmt | yield_stmt )
  def break_stmt[_p: P] = P( kw("break") ).map(_ => Ast.stmt.Break)
  def continue_stmt[_p: P] = P( kw("continue") ).map(_ => Ast.stmt.Continue)
  def return_stmt[_p: P] = P( kw("return") ~~ " ".rep ~~ testlist.map(tuplize).? ).map(Ast.stmt.Return.apply)

  def yield_stmt[_p: P] = P( yield_expr ).map(Ast.stmt.Expr.apply)
  def raise_stmt[_p: P]: P[Ast.stmt.Raise] = P( kw("raise") ~~ " ".rep ~~test.? ~ ("," ~ test).? ~ ("," ~ test).? ).map((Ast.stmt.Raise.apply _).tupled)
  def import_stmt[_p: P]: P[Ast.stmt] = P( import_name | import_from )
  def import_name[_p: P]: P[Ast.stmt.Import] = P( kw("import") ~ dotted_as_names ).map(Ast.stmt.Import.apply)
  def import_from[_p: P]: P[Ast.stmt.ImportFrom] = {
  ...
  def import_as_name[_p: P]: P[Ast.alias] = P( NAME ~ (kw("as") ~ NAME).? ).map((Ast.alias.apply _).tupled)
  def dotted_as_name[_p: P]: P[Ast.alias] = P( dotted_name.map(x => Ast.identifier(x.map(_.name).mkString("."))) ~ (kw("as") ~ NAME).? ).map((Ast.alias.apply _).tupled)
  def import_as_names[_p: P] = P( import_as_name.rep(1, ",") ~ (",").? )
  def dotted_as_names[_p: P] = P( dotted_as_name.rep(1, ",") )
  def dotted_name[_p: P] = P( NAME.rep(1, ".") )
  def global_stmt[_p: P]: P[Ast.stmt.Global] = P( kw("global") ~ NAME.rep(sep = ",") ).map(Ast.stmt.Global.apply)
  def exec_stmt[_p: P]: P[Ast.stmt.Exec] = P( kw("exec") ~ expr ~ (kw("in") ~ test ~ ("," ~ test).?).? ).map {
  ...
  def assert_stmt[_p: P]: P[Ast.stmt.Assert] = P( kw("assert") ~ test ~ ("," ~ test).? ).map((Ast.stmt.Assert.apply _).tupled)

  def compound_stmt[_p: P]: P[Ast.stmt] = P( if_stmt | while_stmt | for_stmt | try_stmt | with_stmt | decorated )
  def if_stmt[_p: P]: P[Ast.stmt.If] = {
  ...
  def space_indents[_p: P] = P( spaces.repX ~~ " ".repX(indent) )
  def while_stmt[_p: P] = P( kw("while") ~/ test ~ ":" ~~ suite ~~ (space_indents ~~ kw("else") ~/ ":" ~~ suite).?.map(_.toSeq.flatten) ).map((Ast.stmt.While.apply _).tupled)
  def for_stmt[_p: P]: P[Ast.stmt.For] = P( kw("for") ~/ exprlist ~ kw("in") ~ testlist ~ ":" ~~ suite ~~ (space_indents ~ kw("else") ~/ ":" ~~ suite).? ).map {
    case (itervars, generator, body, orelse) =>
      Ast.stmt.For(tuplize(itervars), tuplize(generator), body, orelse.toSeq.flatten)
  }
  def try_stmt[_p: P]: P[Ast.stmt]= {
  ...
  def with_stmt[_p: P]: P[Ast.stmt.With] = P( kw("with") ~/ with_item.rep(1, ",")~ ":" ~~ suite ).map{
    case (items, body) =>
  ...
  def with_item[_p: P]: P[(Ast.expr, Option[Ast.expr])] = P( test ~ (kw("as") ~ expr).? )
  // NB compile.c makes sure that the default except clause is last
  def except_clause[_p: P] = P( space_indents ~ kw("except") ~/ (test ~ ((kw("as") | ",") ~ test).?).? )


  def suite[_p: P]: P[Seq[Ast.stmt]] = {
    def deeper: P[Int] = {
