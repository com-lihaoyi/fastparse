...
class Statements(indent: Int) {
  ...
  val decorator: P[Ast.expr] = P("@" ~/ dotted_name ~ ("(" ~ arglist ~ ")").? ~~ Lexical.nonewlinewscomment.? ~~ NEWLINE).map {
    ...
    case (name, Some((args, keywords))) =>
      val x = collapse_dotted_name(name)
      Ast.expr.Call(x, args, keywords)
  }
  ...
  val classdef: P[Seq[Ast.expr] => Ast.stmt.ClassDef] =
    P(kw("class") ~/ NAME ~ ("(" ~ arglist.? ~ ")").?.map(_.flatten) ~ ":" ~~ suite).map {
      case (name, b, c) =>
        val (args, keywords) = b.getOrElse((Nil, Nil))
        Ast.stmt.ClassDef(name, args, keywords, c, _)
    }

  /// async_funcdef: ASYNC funcdef
  val async_funcdef: P[Seq[Ast.expr] => Ast.stmt.AsyncFunctionDef] = P(kw("async") ~ funcdef).map(f => a => Ast.stmt.AsyncFunctionDef(f(a)))

  /// funcdef: 'def' NAME parameters ['->' test] ':' suite
  val funcdef: P[Seq[Ast.expr] => Ast.stmt.FunctionDef] = P(kw("def") ~/ NAME ~ parameters ~ ("->" ~ test).? ~ ":" ~~ suite).map {
    case (name, args, returns, suite) => Ast.stmt.FunctionDef(name, args, suite, _, returns)
  }
  ...
  val expr_stmt: P[Ast.stmt] = {
    val ann = P(testlist_star_expr ~ annassign).map { case (a, (b, c)) =>
      val simple = if (b.isInstanceOf[Ast.identifier]) 1 else 0
      Ast.stmt.AnnAssign(tuplize(a), b, c, simple)
    }
    val aug = P(testlist_star_expr ~ augassign ~ (yield_expr | testlist.map(tuplize))).map {
      case (a, b, c) => Ast.stmt.AugAssign(tuplize(a), b, c)
    }
    val assign = P(testlist_star_expr ~ ("=" ~ (yield_expr | testlist_star_expr.map(tuplize))).rep).map {
  ...
  /// annassign: ':' test ['=' test]
  val annassign: P[(Ast.expr, Option[Ast.expr])] = P(":" ~ test ~ ("=" ~ test).?)
  ...
  // todo
  val raise_stmt: P[Ast.stmt.Raise] = {
    val cause = P("from" ~ test)
    P(kw("raise") ~ " ".rep ~ (test ~ cause.?).?).map {
      case None => Ast.stmt.Raise(None, None)
      case Some((e, f)) => Ast.stmt.Raise(Some(e), f)
    }
  }
  ...
  val async_stmt: P[Ast.stmt] = {
    val async_funcdef_stmt = P(async_funcdef).map(f => f(Nil))
    val async_with_stmt = P( "async" ~ with_stmt ).map(AsyncWith)
    val async_for_stmt = P( "async" ~ for_stmt).map(AsyncFor)
    P( async_funcdef_stmt | async_with_stmt | async_for_stmt )
  }
  ...
  val try_stmt: P[Ast.stmt] = {
    ...
    P(`try` ~~ excepts ~~ `else`.? ~~ `finally`.?).map { case (body, handlers, orelse, finally_) =>
      Ast.stmt.Try(body, handlers, orelse.getOrElse(Nil), finally_.getOrElse(Nil))
    }
  }
  ...
  val with_stmt: P[Ast.stmt.With] = {
    val with_item = P(test ~ (kw("as") ~ expr).?).map(Ast.withitem.tupled)

    P(kw("with") ~/ with_item.rep(1, ",") ~ ":" ~~ suite).map(Ast.stmt.With.tupled)
  }


  /// except_clause: 'except' [test ['as' NAME]]
  // NB compile.c makes sure that the default except clause is last
  val except_clause = P(space_indents ~ kw("except") ~/ (test ~ (kw("as") ~ NAME).?).?)
