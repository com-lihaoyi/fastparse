...
object Ast{
  ...
  object stmt{
    ...
    case class AsyncFunctionDef(funcDef: FunctionDef) extends stmt
    ...
    case class Async(stmt: Ast.stmt) extends stmt
  ...
  object expr{
    ...
    case class Dict(items: Seq[DictItem]) extends expr
    case class Set(elts: Seq[expr]) extends expr
    case class SetComp(elt: expr, generator: comprehension) extends expr
    case class DictComp(key: expr, value: expr, generator: comprehension) extends expr
    case class GeneratorExp(elt: expr, generator: comprehension) extends expr
    ...
    case class Await(expr: expr) extends expr
    trait DictItem extends expr
    case class StarStarExpr(expr: expr) extends DictItem
    case class KeyValueExpr(key: expr, value: expr) extends DictItem
  }
  // col_offset is the byte offset in the utf8 string the parser uses
  case class attributes(lineno: Int, col_offset: Int)
  ...
  case object operator{
    ...
    case object MatrixMult  extends operator
  ...
  case class comprehension(target: expr, test: expr, iter: Option[expr], async: Boolean = false) extends expr
  case class ifExpr(test: expr, iter: Option[expr]) extends expr
  case class testlist(list: Seq[Ast.expr]) extends expr
  ...
  case class arguments(args: Seq[expr.Name], vararg: Option[expr.Name], named_args: Seq[expr.Name],
                            kwarg: Option[expr.Name], defaults: Seq[expr], named_defaults: Seq[expr])

  case object arguments {
    val empty = arguments(Seq(), None, Seq(), None, Seq(), Seq())
  }
