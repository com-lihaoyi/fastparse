...
object Expressions {
  ...
  object LoadExpr extends ExprCtx(expr_context.Load)

  object StoreExpr extends ExprCtx(expr_context.Store)

  class ExprCtx(context: expr_context) {
    def tuplize(xs: Seq[Ast.expr]) = xs match {
      case Seq(x) => x
      case xs => Ast.expr.Tuple(xs, context)
    }

    def tuplize(xs: Seq[Ast.expr], localContext: Ast.expr_context) = xs match {
      case Seq(x) => x
      case xs => Ast.expr.Tuple(xs, localContext)
    }
    ...
    /// star_expr: '*' expr
    val star_expr: P[Ast.expr.Starred] = P( "*" ~ expr ).map(expr => Ast.expr.Starred(expr, context))
    ...
    val atom: P[Ast.expr] = {
      ...
      P(
        empty_tuple  |
          ...
          NAME.map(id => Ast.expr.Name(id, context)) |
          NUMBER |
          ELLIPSIS
      )
    }
    ...
    case object ListFactory extends SeqFactory {
      def seqToExpr(seq: Seq[Ast.expr], trailingComma: Option[String]) = Ast.expr.List(seq, context)
    ...
    val trailer: P[Ast.expr => Ast.expr] = {
      ...
      val slice = P("[" ~ subscriptlist ~ "]").map(args => (lhs: Ast.expr) => Ast.expr.Subscript(lhs, args, context))
      val attr = P("." ~ NAME).map(id => (lhs: Ast.expr) => Ast.expr.Attribute(lhs, id, context))
      P( call | slice | attr )
    }
    ...
    val argument: P[Either[Ast.expr, Ast.keyword]] = {
      ...
      val arg_star = P( "*" ~ test).map(x => Left(Ast.expr.Starred(x, context)))
    ...
    val comp_for: P[Seq[Ast.comprehension]] = {
      ...
      P( "async".!.? ~ "for" ~ StoreExpr.exprlist ~ "in" ~ or_test ~ comp_iter.? ).map{
    ...
    val yield_expr: P[Ast.expr.Yield] = P( kw("yield") ~ yield_arg.? ).map {
      ...
      case Some(list) =>
        val expr = if (list.size == 1) list.head else Ast.expr.Tuple(list, context)
