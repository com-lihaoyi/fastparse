...
object ExampleTests extends TestSuite{
  ...
  val tests = Tests{
    test("basic"){
      ...
      test("option"){
        def option[$: P] = P( "c".? ~ "a".rep(0, "b", Int.MaxValue, -1).! ~ End)
...
//        assert(parser == !(" "))
      ...
      test("flatMap"){
        ...
        def xml[$: P] = P( leftTag.flatMap(rightTag) )
    ...
    test("cuts"){
      ...
      test("delimiternocut"){
        ...
        def tuple[$: P] = P( "(" ~ digits.!.rep(0, ",", Int.MaxValue, -1) ~ ")" )
