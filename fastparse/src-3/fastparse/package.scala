...
package object fastparse {
  ...
  ): ParsingRun[T] = parser(new ParsingRun(
    input = input,
  ...
  inline def P[T](inline t: P[T])(implicit name: sourcecode.Name, ctx: P[_]): P[T] =
    NonMarcroImpls.pInline(t)(name, ctx)
  ...
  extension [T](inline parse0: P[T])
    ...
    inline def filter(f: T => Boolean)(using ctx: P[Any]): P[T] =
      NonMarcroImpls.filterInline[T](parse0)(f)(ctx)
    ...
    inline def |[V >: T](inline other: P[V])(using ctx: P[Any]): P[V] =
      NonMarcroImpls.eitherInline[T, V](parse0)( other)(ctx)

    /** Plain cut operator. Runs the parser, and if it succeeds, backtracking
      * past that point is now prohibited
      */
    inline def /(implicit ctx: P[_]): P[T] = NonMarcroImpls.cutInline[T](parse0)(ctx) // macro MacroImpls.cutMacro[T]
    ...
    inline def ?[V](implicit optioner: Implicits.Optioner[T, V], ctx: P[Any]): P[V] =
      NonMarcroImpls.optionInline[T, V](parse0)(optioner, ctx)
    ...
    /** Transforms the result of this parser using the given function. Useful
      ...
      */
    inline def map[V](inline f: T => V): P[V] = NonMarcroImpls.mapInline[T, V](parse0)(f)
    ...
    /** Transforms the result of this parser using the given partial function,
      ...
      */
    inline def collect[V](inline f: PartialFunction[T, V]): P[V] = NonMarcroImpls.collectInline[T, V](parse0)(f)
    ...
    /** Transforms the result of this parser using the given function into a
      ...
      */
    inline def flatMapX[V](inline f: T => P[V]): P[V] = NonMarcroImpls.flatMapXInline[T, V](parse0)(f)
    ...
    inline def flatMap[V](f: T => P[V])(using whitespace: P[Any] => P[Unit]): P[V] =
      NonMarcroImpls.flatMapInline[T, V](parse0)(f)(whitespace)
    ...
    /** Capture operator; makes the parser return the span of input it parsed
      ...
      * [[map]] or [[flatMapX]]
      */
    inline def !(using ctx: P[Any]): P[String] = NonMarcroImpls.captureInline(parse0)(ctx)
  end extension
  ...
  extension [T](parse0: => P[T])
    ...
    def ~/[V, R](other: => P[V])(implicit
        ...
        ctx: P[_]
    ): P[R] = NonMarcroImpls.parsedSequence0[T, V, R](() => parse0)(() => other, true)(s, Some(whitespace), ctx)
    ...
    def ~[V, R](other: => P[V])(implicit
        ...
        whitespace: P[Any] => P[Unit],
        ctx: P[_]
    ): P[R] = NonMarcroImpls.parsedSequence0[T, V, R](() => parse0)(() => other, false)(s, Some(whitespace), ctx)
    ...
    def ~~/[V, R](other: => P[V])(implicit s: Implicits.Sequencer[T, V, R], ctx: P[_]): P[R] =
      NonMarcroImpls.parsedSequence0[T, V, R](() => parse0)(() => other, true)(s, None, ctx)
    ...
    /** Raw repeat operator; runs the LHS parser 0 or more times *without*
      ...
      * index of the last run.
      */
    def repX[V](using repeater: Implicits.Repeater[T, V], ctx: P[Any]): P[V] = repX()
  ...
  inline def CharPred(inline p: Char => Boolean)(implicit ctx: P[_]): P[Unit] = NonMarcroImpls.charPredInline(p)(ctx)

  /** Parses a single character in one of the input strings representing
    * character classes
    */
  def CharIn(s: String*)(implicit ctx: P[_]): P[Unit] =
    NonMarcroImpls.charInNonMacro(s: _*)(ctx) // macro MacroImpls.charInMacro
  /** Parses one or more characters as long as they are contained
    * in one of the input strings representing character classes
    */
  def CharsWhileIn(s: String)(implicit ctx: P[_]): P[Unit] = NonMarcroImpls.charsWhileInMacro(s, 1)(ctx)

  /** Parses `min` or more characters as long as they are contained
    * in one of the input strings representing character classes
    */
  def CharsWhileIn(s: String, min: Int)(implicit ctx: P[_]): P[Unit] = NonMarcroImpls.charsWhileInMacro(s, min)(ctx)
  ...
  inline def CharsWhile(inline p: Char => Boolean, min: Int = 1)(implicit ctx: P[_]): P[Unit] =
    NonMarcroImpls.charsWhileInline(p, min)(ctx)
  ...
  inline def StringIn(inline s: String*)(implicit ctx: P[_]): P[Unit] =
    ${ NonMarcroImpls.stringInNonMacro0('false, 's)('ctx) }
  ...
  inline def StringInIgnoreCase(inline s: String*)(implicit ctx: P[_]): P[Unit] =
    ${ NonMarcroImpls.stringInNonMacro0('true, 's)('ctx) }
