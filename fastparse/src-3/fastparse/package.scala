...
package object fastparse {
  ...
  implicit class EagerOps[T](val parse0: P[T]) {
    ...
    /** Transforms the result of this parser using the given function. Useful
      ...
      */
    def map[V](f: T => V): P[V] = NonMarcroImpls.mapNonMacro(parse0)(f)
    ...
    /** Transforms the result of this parser using the given partial function,
      ...
      */
    def collect[V](f: PartialFunction[T, V]): P[V] = NonMarcroImpls.collectNonMacro(parse0)(f)
    ...
    /** Transforms the result of this parser using the given function into a
      ...
      */
    def flatMap[V](f: T => P[V])(implicit whitespace: P[Any] => P[Unit]): P[V] =
      NonMarcroImpls.flatMapNonMacro(parse0)(f)(whitespace)
    ...
    /** Transforms the result of this parser using the given function into a
      ...
      * you next want to parse an array, dictionary or string.
      */
    def flatMapX[V](f: T => P[V]): P[V] = NonMarcroImpls.flatMapXNonMacro(parse0)(f)
  ...
  implicit def ByNameOpsStr(parse0: String)(implicit ctx: P[Any]): fastparse.ByNameOps[Unit] =
    ByNameOps(LiteralStr(parse0)) // macro MacroImpls.byNameOpsStrMacro
  ...
  class ByNameOps[T](val parse0: () => P[T]) {
    ...
    /** Plain cut operator. Runs the parser, and if it succeeds, backtracking
      ...
      */
    def /(implicit ctx: P[_]): P[T] = NonMarcroImpls.cutNonMacro(parse0)(ctx) // macro MacroImpls.cutMacro[T]
    ...
    /** Capture operator; makes the parser return the span of input it parsed
      ...
      * [[map]] or [[flatMapX]]
      */
    def !(implicit ctx: P[Any]): P[String] = NonMarcroImpls.captureNonMacro(parse0)(ctx)
    ...
    /** Repeat operator; runs the LHS parser at least `min` to at most `max`
      ...
      */
    def rep[V](min: Int = 0, sep: => P[_] = null, max: Int = Int.MaxValue, exactly: Int = -1)(implicit
  ...
  def CharPred(p: Char => Boolean)(implicit ctx: P[_]): P[Unit] = NonMarcroImpls.charPredNonMacro(p)(ctx)
  ...
  /** Parses one or more characters as long as they satisfy the given
    * predicate
    */
  def CharsWhile(p: Char => Boolean)(implicit ctx: P[_]): P[Unit] = NonMarcroImpls.charsWhileMacro(p, 1)(ctx)

  /** Parses `min` or more characters as long as they satisfy the given
    * predicate
    */
  def CharsWhile(p: Char => Boolean, min: Int)(implicit ctx: P[_]): P[Unit] =
    NonMarcroImpls.charsWhileMacro(p, min)(ctx)
