...
package object fastparse {
  ...
  def IgnoreCase(s: String)(implicit ctx: P[Any]): P[Unit] = {
    ...
    if (ctx.verboseFailures) ctx.setMsg(() => Util.literalize(s))
    res
  }
  ...
  class ByNameOps[T](val parse0: () => P[T]) extends AnyVal{
    ...
    def unary_!(implicit ctx: P[Any]) : P[Unit] = {
      ...
      if (ctx.verboseFailures) ctx.setMsg(() => "!" + Util.parenthize(msg))
      res.cut = startCut
      res
    }
  ...
  def &(parse: => P[_])(implicit ctx: P[_]): P[Unit] = {
    ...
    if (ctx.verboseFailures) ctx.setMsg(() =>
      msg match{
  ...
  def End(implicit ctx: P[_]): P[Unit] = {
    ...
    if (ctx.verboseFailures) ctx.setMsg(() => "end-of-input")
    res
  ...
  def Start(implicit ctx: P[_]): P[Unit] = {
    ...
    if (ctx.verboseFailures) ctx.setMsg(() => "start-of-input")
    res
  }
  ...
  def AnyChar(implicit ctx: P[_]): P[Unit] = {
    ...
    if (ctx.verboseFailures) ctx.setMsg(() => "any-character")
    res
  }
  ...
  def CharPred(p: Char => Boolean)(implicit ctx: P[_]): P[Unit] = {
    ...
    if (ctx.verboseFailures) ctx.setMsg(() => "character-predicate")
    res
  }
  /**
  ...
  def CharsWhile(p: Char => Boolean, min: Int = 1)(implicit ctx: P[_]): P[Unit] = {
    ...
    if (ctx.verboseFailures) ctx.setMsg(() => s"chars-while($min)")
    res
  }
