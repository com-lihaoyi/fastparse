    ...
    q"""
      $ctx match{ case $ctx1 =>
        ...
        def $rec($startIndex: _root_.scala.Int,
          ...
          if (!$ctx1.isSuccess) {
            if ($ctx1.tracingEnabled) $ctx1.shortFailureMsg = () => $parsedMsg() + ".rep"
  ...
  def repX[V](min: Int = 0,
    ...
    @tailrec def rec(startIndex: Int, count: Int, precut: Boolean): ParsingRun[V] = {
      ...
      if (count == 0 && actualMax == 0) ctx.freshSuccess(
        ...
        originalCut |= ctx.cut
        if (!ctx.isSuccess) {
          if (ctx.tracingEnabled) ctx.shortFailureMsg = () => parseMsg() + ".rep"
          ...
          if (nextCount == actualMax) {
            if (ctx.tracingEnabled) ctx.shortFailureMsg = () => parseMsg() + ".rep"
            end(beforeSepIndex, beforeSepIndex, nextCount)
          }
          else {
            ctx.cut = false
