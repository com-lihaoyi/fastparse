  ...
  val tests = this {
    'basic {
      ...
      'test1 {
        ...
        assert(PrettyPrinter.printRuleList(CssParser.ruleList.parse(input).get.value).trim == output.trim)
      }
      ...
      'test3 {
        assert(PrettyPrinter.printRuleList(CssParser.ruleList.parse(output).get.value).trim == output.trim)
      }
    }
  }
}
